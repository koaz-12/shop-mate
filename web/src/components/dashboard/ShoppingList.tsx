...
{/* Pantry Smart Sheet */ }
{
    selectedPantryItem && (
        <PantryItemSheet
            ...
            onConsume={(amount) => {
                const currentQty = parseInt(selectedPantryItem.quantity || "1");
                if (isNaN(currentQty)) {
                    // If not a number, simple restock logic or treat as 1?
                    // Treat as 1 -> 0 -> Restock
                ...
                if (newQty === 0) {
                    toggleItem(selectedPantryItem.id, true); // Move to List
                    setSelectedPantryItem(null);
                } else {
                    updateItemDetails(selectedPantryItem.id, { quantity: newQty.toString() });
                }
            }}
            onRestock={() => {
                toggleItem(selectedPantryItem.id, true);
            ...
            onUpdate={(updates) => {
                updateItemDetails(selectedPantryItem.id, updates);
            }}
        />
    )
}
